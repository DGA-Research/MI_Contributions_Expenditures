...
from pa_txt_parser import parse_txt_bytes
...
parser_selection = st.radio(
    ...
    options=(
        ...
        "Pennsylvania TXT to CSV Converter",
...
workflow_file_types = {
    "Pennsylvania TXT to CSV Converter": ["txt"],
}
file_types = workflow_file_types.get(parser_selection, ["pdf"])

file_type_help = "Accepted file types: " + ", ".join(file_types)

uploaded_file = st.file_uploader(
    "Upload a file for the selected workflow",
    type=file_types,
...
if uploaded_file is None:
    st.info("Select a workflow and upload the corresponding file to begin parsing.")
...
if uploaded_file is not None:
    report_stem = (
        Path(uploaded_file.name).stem if getattr(uploaded_file, "name", None) else "report"
    )
    pdf_workflows = {
        "Michigan Campaign Report Summary and Schedules",
        "Arizona Campaign Finance Report",
    ...
    try:
        if parser_selection in pdf_workflows:
        ...
        elif parser_selection == "Pennsylvania TXT to CSV Converter":
            with st.spinner("Converting text disclosure to CSV..."):
                df = parse_txt_bytes(uploaded_file.getvalue()).fillna("")

            st.success(f"Parsed {len(df)} rows across {len(df.columns)} columns.")
            _render_dataframe_tabs([("Converted TXT", df)])

            csv_bytes = df.to_csv(index=False).encode("utf-8-sig")
            st.download_button(
                "Download CSV",
                data=csv_bytes,
                file_name=f"{report_stem}.csv",
                mime="text/csv",
            )

        else:
            st.error("Unsupported workflow selection.")
